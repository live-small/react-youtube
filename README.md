...
> 한줄정리: 단건조회, 복수조회 로직을 분리할까? 합칠까?

-   상황
    -   비동기 로직 재사용을 위해 class method로 쪼개서 작성했음
    -   youtube의 videoId가 있으면 video정보를 가져올 수 있음
    -   videoId에 ","(comma)로 분리된 string를 넘길 경우, 1개 이상의 video 정보를 api 1회 호출로 가져올 수 있음
    -   그래서 api 호출 횟수를 줄이고자 comma로 분리된 videoId를 넘기고, videoType[]을 리턴받도록 했는데.. 단건 조회를 해야할 경우엔 어떻게 해야할까?
        -   단건 조회, 복수 조회를 하나의 메서드로 할 수 없을까? 같은 로직이고 리턴타입만 다른데, 새롭게 만드는 건 좋은 생각이 아닌 거 같음
        -   근데.. 하나의 메서드에서 2개를 처리하면, 이용하는 곳에서는 어떻게 처리해서 이용할 것인가
-   생각
    -   기본 리턴타입 배열 처리 통일, 단건 조회일 때도 배열에 넣어서 리턴하고, 배열로 핸들링하기
